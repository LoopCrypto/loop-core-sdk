...
describe("Token API", () => {
  ...
  test("should fetch tokens with query parameters", async () => {
    const queryParams: TokenQueryParams = { networkId: "137", page: 1, limit: 10 };
    const mockResponse: TokenResponse = {
      totalResults: 1,
      tokens: [
        {
    ...
    expect(requestMock).toHaveBeenCalledWith("tokens?networkId=137&page=1&limit=10", { method: "GET" });
    expect(result).toEqual(mockResponse);
  });
