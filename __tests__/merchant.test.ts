...
describe("Merchant API", () => {
  ...
  beforeEach(() => {
    ...
    (merchantApi as unknown as { request: jest.Mock }).request = requestMock;
  });
  ...
  test("should fetch merchant list", async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith("merchants?page=1&limit=10", { method: "GET" });
  ...
  test("should retrieve a merchant by ID", async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith(`merchant/${merchantId}`, { method: "GET" });
    expect(result).toEqual(mockResponse);
  });
