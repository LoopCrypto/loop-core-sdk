...
describe("PaymentType API", () => {
  ...
  beforeEach(() => {
    ...
    (paymentTypeApi as unknown as { request: jest.Mock }).request = requestMock;
  });
  ...
  test("should fetch payment types", async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith("payment-types?page=1&limit=10", { method: "GET" });
  ...
  test("should delete a payment type", async () => {
    ...
    const mockResponse: PaymentTypResponse = { totalResults: 0, paymentTypes: [] };

    requestMock.mockResolvedValue(mockResponse);

    const result = await paymentTypeApi.delete(merchantId, tokenId);
    expect(requestMock).toHaveBeenCalledWith(`payment-type/${merchantId}/${tokenId}`, { method: "DELETE" });
    expect(result).toEqual(mockResponse);
  });
});
