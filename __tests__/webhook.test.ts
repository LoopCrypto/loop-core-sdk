...
describe("WebHook API", () => {
  ...
  test("should fetch webhooks with query parameters", async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith("webhooks?page=1&limit=10", { method: "GET" });
    expect(result).toEqual(mockResponse);
  });
  ...
  test("should update an existing webhook", async () => {
    ...
    const queryParams: WebHooksUpdateQueryParams = { networkIds: "137", events: "payment.processed" };
    ...
    expect(requestMock).toHaveBeenCalledWith("webhooks?networkIds=137&events=payment.processed", {
      data: requestBody,
      method: "PATCH",
    });
    expect(result).toEqual(mockResponse);
  ...
  test("should delete a webhook", async () => {
    ...
    const mockResponse: WebHooksResponse = { totalResults: 0, webhooks: [] };

    requestMock.mockResolvedValue(mockResponse);

    const result = await webhookApi.delete(webhookId);
    expect(requestMock).toHaveBeenCalledWith(`webhook/${webhookId}`, { method: "DELETE" });
    expect(result).toEqual(mockResponse);
  });

  test("should generate a new webhook secret", async () => {
    const mockResponse: WebHookSecretResponse = { secret: "new-secret-key" };

    requestMock.mockResolvedValue(mockResponse);

    const result = await webhookApi.generateSecret();
    expect(requestMock).toHaveBeenCalledWith("webhook/secret", { method: "PUT" });
    expect(result).toEqual(mockResponse);
  });

  test("should retrieve the webhook secret", async () => {
    const mockResponse: WebHookSecretResponse = { secret: "current-secret-key" };

    requestMock.mockResolvedValue(mockResponse);

    const result = await webhookApi.getSecret();
    expect(requestMock).toHaveBeenCalledWith("webhook/secret", { method: "GET" });
    expect(result).toEqual(mockResponse);
  });
