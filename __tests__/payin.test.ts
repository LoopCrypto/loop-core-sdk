...
describe("Payin API", () => {
  ...
  test("should fetch payin list", async () => {
      ...
      payins: [
        {
          ...
          paymentMethod: {
            ...
            preAuthorization: { balance: "500", authorization: "200" },
            status: "ok",
          },
          payoutDestination: {
            networkId: 1,
    ...
    expect(requestMock).toHaveBeenCalledWith("payins?page=1&limit=10", { method: "GET" });
  ...
  test("should retrieve a payin by ID", async () => {
    ...
    expect(requestMock).toHaveBeenCalledWith(`payin/${payinId}`, { method: "GET" });
    expect(result).toEqual(mockResponse);
  });
});
